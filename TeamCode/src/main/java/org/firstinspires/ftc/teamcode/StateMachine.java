...
//    <<------------------------------ Mecanum Wheels --------------------------------->>
    ...
    void translate(double degrees, double power, double distance) { //Degrees0->Straight, Degrees 90 -> Left , Degrees -90 -> Right, Degrees 180 -> Backwards
        ...
        if(next_state_to_execute()) {
            ...
            if (!rbt.motors.get(wheelSet1[0]).isBusy()||(rbt.hasMotorEncoderReached(wheelSet1[0], wheelSetEncoder1) && (rbt.hasMotorEncoderReached(rbt.wheelSet2[1], wheelSetEncoder2)))) {
